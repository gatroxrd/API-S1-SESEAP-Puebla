								...
								if [ $executarMontado -gt 0 ];
									then
								        ...
								        echo -e "\033[33mID de la imagen dentro del contenedor Docker es:\033[0m"
												...
												sudo docker run --hostname -it --rm -e HOST_IP=$(ip -o route get to 8.8.8.8 | sed -n 's/.*src \([0-9.]\+\).*/\1/p') --name dotnet -p $deploymentPort:80 -d dotnet --link mongo:mongo --rm mongo sh -c 'exec mongo "$MONGO_PORT_27017_TCP_27017_TCP_PORT/PMS_DB"'
												fi
