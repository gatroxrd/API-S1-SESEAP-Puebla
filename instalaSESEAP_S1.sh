						    ...
						    *)
								...
								if [ $banderamongoHostname -gt 0 ];
									then
										...
										cd ..
										find / -name parametrosConfiguracion.txt -type f -print | xargs sed -i "s/"${tempmongoHostname}"/$mongoHostnameaux/g" parametrosConfiguracion.txt
										cd API.S1.SESEAP
										executarMontado=1
								fi
								...
								if [ $executarMontado -gt 0 ];
									then
												...
												find / -name appsettings.json -type f -print | xargs sed -i "s/apiName/$apiName/g" appsettings.json
